			...
			<c:otherwise>
					...
					<tbody>
						<c:forEach var="order" items="${cart}" varStatus="status">
							<tr>
								...
								<td><img
									src="${pageContext.request.contextPath}/uploaded/${order.grocery.imageURL}"
									alt="Image of ${order.grocery.groceryName}"></td>
								<td>$${order.grocery.grocery_price}</td>
								<td>${order.order_date}</td>
								<td>
									<div class="quantity-buttons">
										<form
											action="${pageContext.request.contextPath}/orders/reduceQuantity"
										...
										<form
											action="${pageContext.request.contextPath}/orders/addQuantity"
											method="post" style="display: inline;">
											<input type="hidden" name="index" value="${status.index}">
											<button type="submit" class="btn btn-light btn-sm">
				...
				<div class="text-center mt-3">
					<a href="${pageContext.request.contextPath}/orders/checkout"
						class="btn btn-primary">Card</a> <a
						href="${pageContext.request.contextPath}/orders/walletcheckout"
						class="btn btn-primary">Wallet</a>
				</div>
			</c:otherwise>
		</c:choose>
